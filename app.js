...
(function () {
  ...
  function compatProbHtml() {
    ...
    return `<div class="card">
      <h2>📊 相性コードの発生確率</h2>
      <div class="hint" style="margin-bottom:12px;">27通りの宿の組み合わせのうち、各コードが何通りを占めるか＝その相性の出やすさです。登録者どうしの実測ともほぼ一致します。</div>
      ${rows.map(r => `<div class="prob-row">
        <div class="prob-line"><span class="compat-code">${r.c}</span><b class="prob-val">${r.p.toFixed(1)}%</b><span class="prob-head">${escapeHtml(info[r.c] ? info[r.c].headline : "")}</span></div>
        <div class="prob-bar-track"><span class="prob-bar-fill" style="width:${(r.p / max * 100).toFixed(1)}%"></span></div>
      </div>`).join("")}
      <div class="hint" style="margin-top:8px; font-size:10.5px;">Gが最多（気の合う友人＝友・衰）、次いでF（繁栄＝危・成）。E・I・J・K・L・M・Nはレアな相性です。</div>
    </div>`;
  }
  ...
  function renderCompatGuide() {
    ...
    if (!box.dataset.ready) {
      ...
      html += `</div>`;
      html += `<div class="card"><h2>🌙 宿曜27宿マスタ</h2><div class="shuku-grid">` +
        SHUKU_MASTER.map(s => `<div class="shuku-cell"><span class="shuku-id">${s.id}</span><b>${s.name}</b><small>${s.yomi}／${s.group}</small></div>`).join("") +
        `</div></div>`;
      box.innerHTML = html;
      box.dataset.ready = "1";
    }
